...
//stream_context_set_default(['http'=>['proxy'=>'den-entbc-001:80']]);
// helper functions
// -----------------------------------------------------------------------------
...
$totalNum=3603;
//
$fp = fopen("url" . $totalNum . ".txt", "a");

if($fp) { 
	echo "file open";
	for($i=1;$i<=$totalNum;$i++) {
		$url = 'http://www.flickriver.com/photos/xxx/?ajax&page=' . $i . '&nocache=' . time();
		...
		curl_setopt($ch, CURLOPT_PROXY, 'den-entbc-001');
		...
		// echo " " .$html. "	1111";
		$imgs = $html->find('img[class=photo-panel-img]');

		$num = count($imgs);
		for ($j = 0; $j < $num; $j++) {
			$element = $imgs[$j];
			fwrite($fp, '{"i": "'.(($i-1)*4+$j+1).'", "lsrc":"'.$element->largesrc.'"},' ); 
		}
		usleep(1000);
